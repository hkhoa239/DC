...
from .rl.agent.MDQLAgent import MDQLAgent
from .rl.Logger import Logger
from pathlib import Path
import datetime
import numpy as np

class MDQLScheduler(Scheduler):
    ...
    def setAgent(self, env, checkpoint_path=""):
        self.save_dir = Path(f"train_mdql") / datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
        self.agent = MDQLAgent(state_dim=env.observation_space.shape, action_dim=env.action_space.n, save_dir=self.save_dir, checkpoint=checkpoint_path)
