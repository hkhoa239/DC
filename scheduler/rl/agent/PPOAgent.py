...
class PPOAgent():
    ...
    def learn(self, last_value_np: np.ndarray, last_done: bool):
        ...
        if last_done:
            ...
            self.actor_critic_net.eval() # Đảm bảo critic ở eval mode
            with torch.no_grad():
                last_state_tensor = torch.tensor(last_value_np, dtype=torch.float32, device=self.device).unsqueeze(0)
                last_value_tensor = self.actor_critic_net.get_value(last_state_tensor) # Shape [1,1]
        ...
        old_states_batch = torch.stack(self.memory_states)       # Shape: (N, state_dim)
        # old_actions_batch = torch.cat(self.memory_actions)     # Shape: (N,)
        old_actions_batch = torch.stack(self.memory_actions)     # Shape: (N,)
        ...
        for _ in range(self.ppo_epochs):
            ...
            new_values = self.actor_critic_net.get_value(old_states_batch).squeeze(-1) # Shape: (N,)
