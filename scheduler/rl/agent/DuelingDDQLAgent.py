...
from network.net import DuelingQNet
import os
import sys
import torch
...
class DuelingDDQLAgent(BaseAgent):
    ...
    def act(self, state):
        if np.random.rand() < self.exploration_rate:
            ...
            value, advantages = self.net(state, model="online")
    ...
    def td_target(self, rewards, states, actions, next_states, dones):
        value_s_dash, advantage_s_dash = self.net(next_states, model="target")
        value_s_dash_eval, advantage_s_dash_eval = self.net(next_states, model="online")

        # print("advanta" , (value_s_dash-advantage_s_dash.mean(dim=1, keepdim=True)).shape)
        ...
        best_action = torch.argmax(q_eval, axis=1)
        # print("best_action", best_action.shape)
        
        q_target = rewards + (1-dones.float()) * self.gamma * q_next[np.arange(self.batch_size), best_action]
        return q_target.float()
