...
class BWGD(Workload):
    def __init__(self, meanNumContainers, sigmaNumContainers):
        ...
        curr_dir = os.path.dirname(os.path,abs(__file__))

        self.dataset_path = os.path.abspath(os.path.join(curr_dir, "../dataset/gwa-bitbrains/fastStorage"))
    ...
    def generateNewContainers(self, interval):
        ...
        for i in range(max(1,int(gauss(self.mean, self.sigma)))):
            ...
            disk_size = self.disk_size[index % len(self.disk_sizes)]
