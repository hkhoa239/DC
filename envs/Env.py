...
class Env():
    ...
    def addWorkloadList(self):
        ...
        # print("ADD TO INACTIVE: ", [vm.id for vm in deployedVms])
        # print("INACTIVE: ", self.inactiveVmId)
    ...
    def assignVmToHost(self, vm:VM, host:Host):
        # print(f"VM {vm.id} ASSIGN TO HOST {host.id}")
        host.allocate(vm)
        vm.updateHost(host)
        vm.handle(action=1)
    ...
    def handleDestroyVms(self):
        ...
        # print("DESTREOYED: ", vmDestroyedByCIDs)
        # print("INACTiVE: ", self.inactiveVmId)
        # print("Active: ", self.activeVmId)
